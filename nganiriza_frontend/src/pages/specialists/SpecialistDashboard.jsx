...
import { Calendar, MessageCircle, Users, BarChart3, Menu, MessageSquare, Clock, CheckCircle, XCircle, CalendarClock } from 'lucide-react';
...
const SpecialistDashboard = () => {
  ...
  const loadPatientData = async (userId) => {
    try {
      ...
      const messagesResponse = await apiClient.get('/api/specialists/messages/inbox/');
      const allMessages = messagesResponse.data?.results || messagesResponse.data || [];
      ...
      const appointmentsResponse = await apiClient.get('/api/specialists/appointments/specialist/');
      const allAppointments = appointmentsResponse.data?.results || appointmentsResponse.data || [];
              ...
              <div className="patients-list">
                {patients.map((patient) => (
                  <button
                    ...
                    className={`patient-item ${selectedPatient?.user_id === patient.user_id ? 'active' : ''} ${patient.has_active_conversation ? 'has-active' : ''}`}
                  >
                    ...
                    <div className="patient-info">
                      <div className="patient-name-row">
                        <span className="patient-name">{patient.user_name || 'Unknown Patient'}</span>
                        {(patient.unread_messages > 0 || patient.pending_appointments > 0) && (
                          <div className="patient-badges">
                            {patient.unread_messages > 0 && (
                              <span className="badge unread">{patient.unread_messages}</span>
                            )}
                            {patient.pending_appointments > 0 && (
                              <span className="badge pending">{patient.pending_appointments}</span>
                            )}
                          </div>
                        )}
                      </div>
          ...
          {!selectedPatient ? (
            <>
              ...
              <div className="dashboard-empty-state">
                ...
                <p>Choose a patient from the sidebar to see their appointments and messages</p>
              </div>
            </>
          ) : (
            <>
                ...
                <div className="patient-detail-header">
                  ...
                  <div className="patient-header-info">
                    ...
                    <div>
                      ...
                      <p className="patient-header-email">{selectedPatient.user_email}</p>
                    </div>
                  </div>
                </div>
